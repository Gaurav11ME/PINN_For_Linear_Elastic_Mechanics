...
def Equation_loss(X,weights, biases,C,sx,sy):
  ...
  dsig_yy_dy = (1.0/sy)*tf.gradients(sig_yy,X)[0][:,1]
...
nu = 0.3
C = tf.Variable([[1.0, nu, 0.0],[nu,1.0,0.0],[0.0, 0.0, (1-nu)]],trainable=False)

pred_uv_1 = neural_net(D_XY_S1, weights_1, biases_1)
pred_uv_2 = neural_net(D_XY_S2, weights_2, biases_2)
uv_ex_1 = ExactSolution(D_XY_1)
uv_ex_2 = ExactSolution(D_XY_2)
[f1_1,f2_1] = Equation_loss(D_XY_S1, weights_1, biases_1, C,sf_x_1,sf_y_1)
[f1_2,f2_2] = Equation_loss(D_XY_S2, weights_2, biases_2, C,sf_x_2,sf_y_2)
...
with tf.Session() as sess:
    sess.run(init)
    UV_EX_1 = sess.run(uv_ex_1, tf_dict_ex_1)
    UV_EX_2 = sess.run(uv_ex_2, tf_dict_ex_2)
    # Loading optimized weights and biases
    saver_w_1.restore(sess, "./2D_ElasticSolid_Notch_DPINN_scaled_rev/my_training_mode_nn1_w1.ckpt")
    saver_b_1.restore(sess, "./2D_ElasticSolid_Notch_DPINN_scaled_rev/my_training_mode_nn1_b1.ckpt")
    saver_w_2.restore(sess, "./2D_ElasticSolid_Notch_DPINN_scaled_rev/my_training_mode_nn1_w2.ckpt")
    saver_b_2.restore(sess, "./2D_ElasticSolid_Notch_DPINN_scaled_rev/my_training_mode_nn1_b2.ckpt")
    UV_NN_1 = sess.run(pred_uv_1, tf_dict_nn_1)
    UV_NN_2 = sess.run(pred_uv_2, tf_dict_nn_2)
    pde_1_1_err = sess.run(f1_1,tf_dict_nn_1)
    pde_2_1_err = sess.run(f2_1,tf_dict_nn_1)
    pde_1_2_err = sess.run(f1_2,tf_dict_nn_2)
    pde_2_2_err = sess.run(f2_2,tf_dict_nn_2)
sess.close()
...
u_pred = np.transpose(np.concatenate([[u_pred_1],[u_pred_2]],axis=1))
v_pred = np.transpose(np.concatenate([[v_pred_1],[v_pred_2]],axis=1))
u_ex = np.transpose(np.concatenate([[u_ex_1],[u_ex_2]],axis=1))
v_ex = np.transpose(np.concatenate([[v_ex_1],[v_ex_2]],axis=1))
...
C1 = uplot_fit.scatter(x,y,marker ='.',c=u_ex)
plt.colorbar(C1)

uplot_fit.set_title('Contours of u_r (Exact)')

fig2, uplot_fit = plt.subplots()
C2 = uplot_fit.scatter(x,y,marker ='.',c=u_pred)
plt.colorbar(C2)
uplot_fit.set_title('Contours of u_r (PINN)')
'''
